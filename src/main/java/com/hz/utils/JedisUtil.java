 ...
 */
	...
	static {
		...
		config.setMaxWaitMillis(1000 * 100);//连接池耗尽最大等待时间 
		config.setTestOnBorrow(true);//在borrow一个jedis实例的时候，是否要进行验证操作，如果赋值true。则得到的jedis实例肯定是可以用的。
		/**
		 * 实例化jedis连接池对象
		 */
		jedisPool = new JedisPool(config, JRedisPoolConfig.inst().getRedisHost(), JRedisPoolConfig.inst().getRedisPort(), JRedisPoolConfig.inst().getRedisTimeout(),
				JRedisPoolConfig.inst().getRedisPass(),JRedisPoolConfig.inst().getRedisDatabase());
	...
	public void expire(String key, int seconds) {
		...
		Jedis jedis = getJedis();//从jedis连接池中获取获取jedis对象 
	...
	public class Keys {
		...
		public boolean exists(String key) {
			// ShardedJedis sjedis = getShardedJedis();
			Jedis sjedis = getJedis();
			boolean exis = sjedis.exists(key);  //参数：key    返回：布尔型回复，如果键存在则为true，否则为false
			returnJedis(sjedis);
			return exis;
		}
